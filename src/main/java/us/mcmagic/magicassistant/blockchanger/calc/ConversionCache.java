...
import us.mcmagic.magicassistant.blockchanger.calc.lookup.SphericalBuffer;
...
public class ConversionCache {
    ...
    public ConversionCache(SegmentLookup defaultLookupTable) {
        ...
        // Duplicate cache
        this.conversionCache = new WeakHashMap<>();
    }
     ...
     */
            int BUFFER_RADIUS = 16;
            cache = new SphericalBuffer<>(BUFFER_RADIUS * 2, BUFFER_RADIUS * 2);
    ...
    private SegmentLookup getCachedConversion(SegmentLookup lookup) {
        ...
        if (previous != null && previous.get() != null) {
            ...
            try {
                lock.writeLock().lock();
                conversionCache.put(lookup, new WeakReference<>(lookup));
            } finally {
